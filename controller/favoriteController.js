...
module.exports= {
    getFavoriteAll: async(req,res)=>{
        try {
            // get favorite data
            const getFavorite = `SELECT f.id, f.user_id, f.product_id, p.name, pc.color, f.qty, f.price_each, pi.image FROM favorite f 
    ...
    getFavoriteDataByID: async(req,res)=>{
        ...
        try {
            // get favorite data
            const getFavorite = `SELECT f.id, f.user_id, f.product_id, p.name, pc.color, p.desc, f.qty, f.price_each, pi.image FROM favorite f 
            JOIN products p ON f.product_id = p.id
            JOIN product_color pc ON f.color_id = pc.id
